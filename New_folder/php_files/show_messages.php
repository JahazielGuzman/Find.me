<?php
     ...
     $sql = "SELECT thumbnail, numsessions AS online, fromuser, touser, SUBSTRING( message, 1, 64 ) AS message, datesent, checked

FROM mess_table LEFT JOIN (SELECT * FROM pic_table WHERE defaultpic = '1') AS NEW ON (NEW.user = IF( fromuser = :user, touser, fromuser )) 


LEFT OUTER JOIN (SELECT numsessions,usern FROM sessions GROUP BY usern) as numsess ON (numsess.usern = IF( fromuser = :user, touser, fromuser ))
