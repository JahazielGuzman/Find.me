     ...
     if (typeof page == 'undefined') {
          ...
          $.each(data,function (pic,showpic) {
               ...
               $('.fancyboxed').fancybox({
		          ...
		          onComplete: function(){
       $('#fancybox-bg-s').html('<p style="background-color:white;"><p id="showcomments"></p><textarea id="insert_comment"></textarea><button type="button" id="sendcom">post</button></p>');
       ...
       str = '';
            $.each(data,function (inc,comm) {
                 str += comm.thumbnail + " " + comm.fromuser + " " + comm.usercomment;     
            });
     ...
     $.post('/php_files/show_pics.php',{'visited':page},function (data) {
          $.each(data,function (pic,showpic) {
               ...
               $('.fancyboxed').fancybox({
		          ...
		          titlePosition: 'inside',
		          onComplete: function(){
       $('#fancybox-bg-s').html('<p style="background-color:white;"><textarea id="insert_comment"></textarea><button type="button" id="sendcom">post</button></p><p id="showcomments"></p>');
       $('#sendcom').click(function () {
       var src = $('#fancybox-img').attr('src');
       var com = $('#insert_comment').val();
       if (com != '') {
       ...
       str = '';
            $.each(data,function (inc,comm) {
                 str += '<table style="overflow: auto;"><tbody><tr><td><img style="' +
          ...
          $('.pic_button').livequery(function () {
               ...
               defordel = $thisn.text();
               $.post('/php_files/change_thumb.php',{'href':href,"group":page,'pic_button':defordel},function (data) {
    ...
    $( "[id*='fancybox']" ).remove();  
     ...
     $fin.livequery(function () {
                $fin.click(function () {
                     ...
                     path = path.find('#crop');
                     path = path.attr('src');
                     $.post('/pics/thumbnail.php',{'group':page,'x':x,'y':y,'width':width,'height':height,'path': path,'bill':bill},
                     function (data) {
                          $fin.html(data);
                          $('#wrapper').html('<link rel="stylesheet" href="/js_files/fancybox/fancybox/jquery.fancybox-1.3.4.css" type="text/css" />'+
                          '<script src="/js_files/fancybox/fancybox/jquery.fancybox-1.3.4.js" type="text/javascript"></script>' +
